...
const { redisClientConfig } = require('../../../utils/constans')
...
const userRepositiry = require('../../user/repository/user-repositiry')
...
class AuthService {
    ...
    async checkAuth(request, reply) {
        ...
        if (userData[0]['permission']) {
            ...
            const menuData = await userRepositiry.generateMenu()
            menuData.forEach(element => {
                if (element?.children?.length > 0) {
                    element.children = element.children.filter(obj => {
                        if (notEmptyAccessId.includes(obj?.module_id)) {

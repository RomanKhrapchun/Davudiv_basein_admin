...
class KindergardenRepository {
    ...
    async getDebtByDebtorId(debtId, displayFieldsUsers) {
        let sql = `select ${displayFieldsUsers.map(field => ` ${field}`)} from ower.kindergarden_debt where id = ?`
    ...
    async getRequisite() {
        return await sqlRequest('select * from ower.kindergarden_settings')
    }

}

module.exports = new KindergardenRepository();